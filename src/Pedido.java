...
public class Pedido {
	...
	private int quantItensPedidos = 0;
     ...
     */
	public boolean incluirItemPedido(Produto novo, int quantidade) {
		ItemDePedido novoItem = new ItemDePedido(novo, quantidade, novo.valorDeVenda());
		if (quantItensPedidos < MAX_PRODUTOS) {
			this.itensDePedido[quantItensPedidos++] = novoItem;
			return true;
		}
		return false;
	}
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantItensPedidos; i++) {
			valorPedido += itensDePedido[i].calcularSubtotal();
		}
	...
	public String toString() {
		...
		stringPedido.append("Pedido com " + quantItensPedidos + " produtos.\n");
	...
	public void mesclarPedido(Pedido outroPedido){
		int tamanhoMescla = this.itensDePedido.length + outroPedido.itensDePedido.length;
		if(tamanhoMescla <= MAX_PRODUTOS){
		this.itensDePedido = juntandoPedidos(this.itensDePedido, outroPedido.itensDePedido);
		this.quantItensPedidos = tamanhoMescla;
		}else{
			throw new IllegalArgumentException("A Mescla de pedidos estrapola a capacidade maxima de produtos por pedido!");
		}
	}

	public ItemDePedido[] juntandoPedidos(ItemDePedido[] a, ItemDePedido[] b){
		int tamanhoA = a.length;
		int tamanhoB = b.length;
		int tamanho = tamanhoA + tamanhoB;
		ItemDePedido[] aux = new ItemDePedido[tamanho];
		for(int i = 0; i< a.length; i++){
			aux[i] = a[i];
		}
		int produtosRepetidos =0;
		for(int i= 0; i< tamanhoB; i++ ){
			if(buscarIProduto(b[i].getProduto())){
				produtosRepetidos++;
			}
			aux[i+(tamanhoB-i)] = b[i+produtosRepetidos];
		}
		return aux;
	}
	public boolean buscarIProduto(Produto produto){

		for (ItemDePedido itemDePedido : itensDePedido) {
			if (itemDePedido.equals(produto)) {
				return true;
			}
		}
		return false;
